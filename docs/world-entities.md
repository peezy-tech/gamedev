...
```bash
vp run world:entities -- <command> [flags]
```
...
```bash
vp run world:entities -- delete --blueprint Tree --yes
```

Delete by explicit ID list file:

```bash
vp run world:entities -- delete --ids tmp/delete-ids.json --yes
```
...
# 2) Run CLI
vp run world:entities -- add --template-id k3sbGG4iq4 --transforms tmp/run-001.json --yes
# 3) Remove temp file
rm tmp/run-001.json
```
