...
- Run `vp run apps:new <AppName>` (creates `apps/<AppName>/` with `index.js` + blueprint)
- Batch-edit `world.json` entities: `vp run world:entities -- add --template-id <id> --transforms <file>` / `vp run world:entities -- delete --blueprint <name>`
- To duplicate one app many times (eg, make a forest from one `Tree`), generate many transforms and run `vp run world:entities -- add --template-id <TreeEntityId> --transforms tmp/forest.json --yes`, then delete `tmp/forest.json`
